...
- Свайп сверху вниз перезагружает страницу, как в браузере. На iOS это делает сам
  WebView (`pullToRefreshEnabled`). На Android `pullToRefreshEnabled` не работает, а
  `RefreshControl` вокруг WebView не получает жест — его перехватывает сам WebView
  (issue #6, см. также react-native-webview#103). Поэтому на Android жест ловит
  инжектированный в страницу детектор overscroll: он шлёт `postMessage` в RN, RN
  перезагружает страницу и показывает свой индикатор поверх WebView. Детектор
  переинъектируется после каждой загрузки (`onLoadEnd`), включая страницы с ошибкой
  сети — там свайп-обновление нужнее всего.
- В правом нижнем углу «почти прозрачными цифрами» показан штамп сборки —
  `versionName` APK (номер + дата сборки, issue #6). На CI он прошивается из
  `github.run_number` и времени запуска (см. `android/app/build.gradle`, переменные
  `BUILD_NUMBER`/`BUILD_DATE`), локальная сборка получает `dev`.
